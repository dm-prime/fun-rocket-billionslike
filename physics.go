...
func (g *Game) updatePhysics(ship *Ship, input ShipInput, dt float64) {
	...
	// Track turning state for visual effects
	if input.TurnLeft {
	...
	// Track thrust state for visual effects
...
func (g *Game) isOnCollisionCourse(player *Ship, rock *Rock, lookAheadTime float64) bool {
	...
	collisionRadius := player.CollisionRadius() + rock.CollisionRadius()
...
// checkBulletToShipCollision checks if a bullet is colliding with a ship
func (g *Game) checkBulletToShipCollision(bullet *Bullet, ship *Ship) bool {
	dx := bullet.pos.x - ship.pos.x
	dy := bullet.pos.y - ship.pos.y
	dist := math.Hypot(dx, dy)

	collisionRadius := bulletRadius + ship.CollisionRadius()
	return dist < collisionRadius
}

// checkBulletToRockCollision checks if a bullet is colliding with a rock
func (g *Game) checkBulletToRockCollision(bullet *Bullet, rock *Rock) bool {
	dx := bullet.pos.x - rock.pos.x
	dy := bullet.pos.y - rock.pos.y
	dist := math.Hypot(dx, dy)

	collisionRadius := bulletRadius + rock.CollisionRadius()
	return dist < collisionRadius
}

// checkBulletCollisions checks all bullets against all ships and rocks and applies damage
func (g *Game) checkBulletCollisions(dt float64) {
	...
	for bulletID, bullet := range g.bullets {
		...
		for shipID, ship := range g.ships {
			...
			// Check collision
			if g.checkBulletToShipCollision(bullet, ship) {
				// Apply damage
		...
		// Check collision with all rocks
		for rockID, rock := range g.rocks {
			// Skip if rock is dead
			if rock.health <= 0 {
				continue
			}

			// Skip if bullet was fired by this rock (rocks don't fire bullets, but for safety)
			if bullet.ownerID == rockID {
				continue
			}

			// Check collision
			if g.checkBulletToRockCollision(bullet, rock) {
				// Apply damage
...
// checkShipCollisions checks all ships against each other and against rocks
func (g *Game) checkShipCollisions(dt float64) {
	...
	for id1, ship1 := range g.ships {
		...
		for id2, ship2 := range g.ships {
			if id1 >= id2 { // Skip self and already-processed pairs
				continue
			}
			...
			// Check collision
			if g.checkShipToShipCollision(ship1, ship2) {
				collisionPairs[key] = true

				// Apply ship-ship collision damage
		...
		for rockID, rock := range g.rocks {
			...
			// Create unique key for collision pair
			key := fmt.Sprintf("s%d-r%d", id1, rockID)
			...
			// Check collision
			if g.checkShipToRockCollision(ship1, rock) {
				collisionPairs[key] = true

				// Apply rock collision damage
				damage := rockCollisionDamage
				ship1.OnCollision(rock, damage)
				rock.OnCollision(ship1, damage)

				// Apply velocity change on collision (bounce)
				dx := rock.pos.x - ship1.pos.x
				dy := rock.pos.y - ship1.pos.y
				dist := math.Hypot(dx, dy)
				if dist > 0.1 {
					// Normalize direction
					dx /= dist
					...
					// Apply impulse (push ship and rock apart)
					impulse := 50.0 * dt
					ship1.vel.x -= dx * impulse
					ship1.vel.y -= dy * impulse
					rock.vel.x += dx * impulse
					rock.vel.y += dy * impulse
				}
			}
		}
	}
...
// checkShipToShipCollision checks if two ships are currently colliding
func (g *Game) checkShipToShipCollision(ship1 *Ship, ship2 *Ship) bool {
	dx := ship1.pos.x - ship2.pos.x
	dy := ship1.pos.y - ship2.pos.y
	dist := math.Hypot(dx, dy)

	collisionRadius := ship1.CollisionRadius() + ship2.CollisionRadius()
	return dist < collisionRadius
}

// checkShipToRockCollision checks if a ship and rock are currently colliding
func (g *Game) checkShipToRockCollision(ship *Ship, rock *Rock) bool {
	dx := ship.pos.x - rock.pos.x
	dy := ship.pos.y - rock.pos.y
	dist := math.Hypot(dx, dy)

	collisionRadius := ship.CollisionRadius() + rock.CollisionRadius()
	return dist < collisionRadius
}

// removeDeadEntities removes dead ships, rocks, and bullets
func (g *Game) removeDeadEntities() {
	// Remove dead ships (except player - keep for game over handling)
	...
	// Remove dead bullets (expired bullets)
	for id, bullet := range g.bullets {
		if !bullet.IsAlive() {
			g.RemoveBullet(id)
		}
	}

	// Update bullets with invalid targets
	for _, bullet := range g.bullets {
		if bullet.isHoming && bullet.targetID != InvalidEntityID {
			...
			if (targetShip == nil || targetShip.health <= 0) && (targetRock == nil || targetRock.health <= 0) {
				// Target is dead or doesn't exist, convert to regular bullet
...
func (g *Game) spawnRockNearPath(player *Ship) {
	...
	// Create rock with small drift velocity using factory
	rock := NewRock(
		spawnPos,
		vec2{
		...
		rand.Float64()*2*math.Pi, // Random initial rotation
	)

	g.rocks[rock.ID()] = rock
}
...
func (g *Game) manageRocks(player *Ship, dt float64) {
	...
	for rockID, rock := range g.rocks {
		...
		if distToPlayer > rockDespawnDistance || !g.isNearPlayerPath(rock.pos, player) {
			rocksToRemove = append(rocksToRemove, rockID)
		}
	}

	// Remove rocks
	for _, rockID := range rocksToRemove {
		g.RemoveRock(rockID)
	}

	// Spawn new rocks if below target count
	g.rockSpawnTimer += dt
	currentRockCount := len(g.rocks)
